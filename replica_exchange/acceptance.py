...
def swap(x_ladder, t_val, a_bar, lam_start, lam_end, n_replicas, compute_eps, i=None, flow=None):
    ...
    for i_pair, (index_t, index_s) in enumerate(pairs):
        ...
        score_interp = score_ladder[index * i_pair + n_replicas * index : index * (i_pair + 1) + n_replicas * index].clone()

        v_tau = v_ladder[index * index_t : index * (index_t + 1)].clone()
        v_s   = v_ladder[index * index_s : index * (index_s + 1)].clone()
        ...
        v_ladder[index * index_t : index * (index_t + 1)] = accept * v_s   + (1 - accept) * v_tau
        v_ladder[index * index_s : index * (index_s + 1)] = accept * v_tau + (1 - accept) * v_s

        # print(f"t {t_val} | lam {lam_ladder[index_t]:.2f}↔{lam_ladder[index_s]:.2f} "
        #         f"| integral {integral.mean():.3f} | acceptance {accept.mean():.3f}")

    return x_ladder[:n_replicas*index], v_ladder[:n_replicas*index]