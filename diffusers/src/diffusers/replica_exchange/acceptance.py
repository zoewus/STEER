...
@torch.no_grad()
def swap(x_ladder, lam_ladder, t_val, a_bar, compute_eps, chunk_size=50000,i=None):
	if i is None:
		offset = int(t_val) % 2
	else:
		offset = int(i) % 2
	...
	for index_t, index_s in pairs:
		...
		idx_tau = torch.argsort(x_tau, stable=True)  # just use first element as sort key
		idx_s = torch.argsort(x_s, stable=True)

		all_x_tau.append(x_tau)  # unsorted view
		all_x_s.append(x_s)
		sorted_idxs_tau.append(idx_tau)
		sorted_idxs_s.append(idx_s)

	# single forward pass over all pairs
	x_tau_cat = torch.cat([x[idx] for x, idx in zip(all_x_tau, sorted_idxs_tau)], dim=0)
	x_s_cat   = torch.cat([x[idx] for x, idx in zip(all_x_s,   sorted_idxs_s)],   dim=0)
